...
@section('title', 'Редактировать шаблон ответа')

@section('content')
<div class="px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center">
        <div class="sm:flex-auto">
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Редактировать шаблон ответа</h1>
            <p class="mt-2 text-sm text-gray-700 dark:text-gray-300">
                Измените существующий шаблон для быстрого ответа клиентам
            </p>
        </div>
    ...
    <div class="mt-8 max-w-2xl">
        <form action="{{ route('admin.response-templates.update', $responseTemplate) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" 
                           name="name" 
                           id="name" 
                           value="{{ old('name', $responseTemplate->name) }}"
                ...
                <div>
                    ...
                    <select name="category" 
                        ...
                        @foreach($categories as $key => $name)
                            <option value="{{ $key }}" {{ old('category', $responseTemplate->category) == $key ? 'selected' : '' }}>
                                {{ $name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea name="content" 
                              ...
                              required>{{ old('content', $responseTemplate->content) }}</textarea>
                ...
                <div class="flex items-center">
                    <input type="checkbox" 
                           ...
                           id="is_active" 
                           value="1"
                           {{ old('is_active', $responseTemplate->is_active) ? 'checked' : '' }}
                ...
                <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
                    <h3 class="text-sm font-medium text-gray-900 dark:text-white mb-2">Статистика использования</h3>
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            <span class="text-gray-500 dark:text-gray-400">Количество использований:</span>
                            <span class="ml-2 font-medium text-gray-900 dark:text-white">{{ $responseTemplate->usage_count }}</span>
                        </div>
                        <div>
                            <span class="text-gray-500 dark:text-gray-400">Создан:</span>
                            <span class="ml-2 font-medium text-gray-900 dark:text-white">{{ $responseTemplate->created_at->format('d.m.Y H:i') }}</span>
                        </div>
                        <div>
                            <span class="text-gray-500 dark:text-gray-400">Последнее обновление:</span>
                            <span class="ml-2 font-medium text-gray-900 dark:text-white">{{ $responseTemplate->updated_at->format('d.m.Y H:i') }}</span>
                        </div>
                        <div>
                            <span class="text-gray-500 dark:text-gray-400">Создал:</span>
                            <span class="ml-2 font-medium text-gray-900 dark:text-white">{{ $responseTemplate->creator->name }}</span>
                        </div>
                    </div>
                </div>
